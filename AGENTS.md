...
- Public-facing content changes must keep metadata, structured data, internal linking, and sitemap/robots behavior aligned with the new page state.
- When UI or copy changes, check repeated surfaces like header, footer, navigation, cards, and CTA sections so stale text does not survive in shared components.
- Verify responsive behavior and accessibility for interactive UI changes; use Playwright/Cypress/Lighthouse scripts when this repo exposes them.
